...
if [ "$#" -lt "5" ]; then
    echo "usage: $0 inpath homo_path output_image number raw zoom sigma_nc"
    echo "example: $0 im_%i.tiff homo_%i.tiff out.tiff 101 1 2 0.7"
    exit 1
fi
...
SIGMANC=$7
if [ -z "$SIGMANC" ]; then
    SIGMANC=0.70710678118 # scale of the gaussian weight (sqrt(2)/2)
fi 
...
echo "Starting data accumulation and blurry image computation"
    combi_ckr $INPATH $HOMOPATH $ind_ini $NUMBER $ZOOM $OUT $order $SIGMANC $RAW
...
echo "Sharpening step"
    ...
    asymptotic_nc_filter $OUT $OUT $order $SIGMANC 1
    crop $cropv $cropv -$cropv -$cropv $OUT $OUT
