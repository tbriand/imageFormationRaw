...
int main(int c, char *v[])
{
    ...
    // read input images
    int w, h, pd;
    double *ref = iio_read_image_double_split(filename_ref, &w, &h, &pd);

    int w2, h2 , pd2;
    double *modified = iio_read_image_double_split(filename_modified, &w2, &h2, &pd2);
    ...
    // write output image
    iio_write_image_double_split(filename_out, out, w2, h2, pd2);
